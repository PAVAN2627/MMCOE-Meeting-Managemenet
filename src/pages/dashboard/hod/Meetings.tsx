...
/** Inline component for HOD to add agenda items to a PRC meeting before it starts */
const AddAgendaItemInline = ({ meeting, user, onSaved }: { meeting: any; user: any; onSaved: () => void }) => {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>Add a new item to this PRC meeting agenda</DialogDescription>
        </DialogHeader>
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            ...
            <Textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Optional description" rows={3} disabled={saving} />
          </div>
          <div className="flex gap-2 pt-2">
            <Button onClick={handleAdd} disabled={saving || !title.trim()} className="flex-1">
...
const HODMeetings = () => {
  ...
  const fetchMeetings = async () => {
    ...
    try {
      ...
      const meetingsList = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as any));
      ...
      for (const taskDoc of tasksSnapshot.docs) {
        ...
        if (assigneeId) {
          ...
          const updatePromise = updateDoc(doc(db, "tasks", taskDoc.id), {
            ...
            assignedToName: assigneeName, // Add the name field
            status: "pending",
...
${transcript}`
      ...
      // Extract meaningful keywords from a string (ignore common words)
    ...
    <DashboardLayout role="HOD Dashboard" navItems={hodNav}>
      ...
      <AlertDialog open={!!deletingMeeting} onOpenChange={(open) => !open && setDeletingMeeting(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setDeletingMeeting(null)}>Cancel</AlertDialogCancel>
